...
const verificaStatiCommesse = async () => {
  try {
    ...
    const [commesse] = await db.query(
      `SELECT id, stati_avanzamento FROM commesse`,
    );
    ...
    for (const commessa of commesse) {
      ...
      for (const reparto of reparti) {
        ...
        if (statiDelReparto.length > 0) {
          ...
          if (attivi.length > 1) {
            ...
            let keep =
              statiDelReparto.find(
                (s) =>
                  s.isActive && normalizeName(s.nome_stato) === TARGET_STATE,
      ...
      if (
        JSON.stringify(statiAggiornati) !== JSON.stringify(statiAvanzamento)
      ) {
        await db.query(
    ...
    console.log(
      `✅ Verifica stati avanzamento completata. Updated: ${updated} | Skipped JSON: ${skipped}`,
    );
  } catch (err) {
...
const allineaStatiCommesse = async () => {
  try {
    ...
    for (const commessa of commesse) {
      ...
      statiAvanzamento = statiAvanzamento.filter((stato) =>
        statiPerReparto.some(
          (s) =>
            Number(s.stato_id) === Number(stato.stato_id) &&
            Number(s.reparto_id) === Number(stato.reparto_id),
        ),
      );
      ...
      const repartiPresenti = [
        ...new Set(statiAvanzamento.map((s) => Number(s.reparto_id))),
      ];

      for (const repartoId of repartiPresenti) {
        const statiReparto = statiAvanzamento.filter(
          (s) => Number(s.reparto_id) === Number(repartoId),
        );
        // 1) se nessun attivo -> accendo In Entrata o fallback
        ...
        if (attivi.length > 1) {
          const keep =
            statiReparto.find(
              (s) => s.isActive && normalizeName(s.nome_stato) === TARGET_STATE,
            ) || attivi.sort((a, b) => (a.ordine ?? 0) - (b.ordine ?? 0))[0];
    ...
    console.log(
      `✅ Allineamento stati avanzamento completato. Updated: ${updated} | Skipped JSON: ${skipped}`,
    );
  } catch (err) {
    console.error(
      "Errore durante l'allineamento degli stati avanzamento:",
      err,
    );
  }
};
